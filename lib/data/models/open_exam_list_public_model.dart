// open_exam_list_public_model.dart

class OpenExamListPublicModel {
  final List<FreeExamPublicModel> items;

  OpenExamListPublicModel({required this.items});

  /// Build from a top-level JSON array (or a wrapped `data` array).
  factory OpenExamListPublicModel.fromJsonList(dynamic json) {
    if (json is List) {
      return OpenExamListPublicModel(
        items: json.map((e) => FreeExamPublicModel.fromJson(_asMap(e))).toList(),
      );
    }
    if (json is Map && json['data'] is List) {
      return OpenExamListPublicModel.fromJsonList(json['data']);
    }
    return OpenExamListPublicModel(items: const []);
  }
