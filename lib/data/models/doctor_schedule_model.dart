...
class Batch {
  ...
  factory Batch.fromJson(Map<String, dynamic> json) => Batch(
    ...
    progressCount: _toInt(json['progress_count']),
  );
...
class ScheduleDate {
  ...
  String get safeTime => time ?? '';
  String get safeDate => date ?? '';
  String get safeDateFormatted => dateFormatted ?? '';
...
class Content {
  ...
  /// NEW (PDF)
  final String? pdf; // raw pdf path (optional)
  final bool? showPdf; // show_pdf
  final String? pdfUrl; // pdf_url (signed / full url)

  /// Solve fields
  final bool? isShowSolve; // now bool in your response (but backend may send 0/1 too)
  ...
  factory Content.fromJson(Map<String, dynamic> json) => Content(
    ...
    // PDF
    pdf: _toString(json['pdf']),
    showPdf: _toBool(json['show_pdf']),
    pdfUrl: _toString(json['pdf_url']),

    // Solve
    isShowSolve: _toBool(json['is_show_solve']),
  ...
  Map<String, dynamic> toJson() => {
    ...
    // PDF (we keep pdf + show_pdf; pdf_url can be regenerated/signed, so OK to omit)
    'pdf': pdf,
    'show_pdf': showPdf,
  ...
  String get safeTopicName => (topicName ?? '').trim();
  String get safeContentStatus => (contentStatus ?? 'unknown').trim();
  String get safeStatusMessage => (statusMessage ?? '').trim();
  String get safeExamStatus => (examStatus ?? '').trim();
  bool get safeIsExamRunning => isExamRunning ?? false;

  String get safePdf => (pdf ?? '').trim();
  bool get safeShowPdf => showPdf ?? false;
  String get safePdfUrl => (pdfUrl ?? '').trim();

  bool get safeIsShowSolve => isShowSolve ?? false;
  ...
  bool get isExamNotCompleted => safeExamStatus.toLowerCase() == 'not completed';

  bool get canShowSolve => safeIsShowSolve;
  bool get canAccessSolve => canShowSolve && safeIsUnlockedSolve;
  bool get hasSolveLinks => safeSolveLinks.isNotEmpty;

  bool get hasPdfMaterials => safeShowPdf && safePdfUrl.isNotEmpty;
}
...
class SolveLink {
  ...
  factory SolveLink.fromJson(Map<String, dynamic> json) => SolveLink(
    id: _toInt(json['id']),
