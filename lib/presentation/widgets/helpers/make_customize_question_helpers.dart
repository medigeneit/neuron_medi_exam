...
class SelectionSummaryCompactCard extends StatelessWidget {
  ...
  final int planCap; // free: min(20, poolCap), premium: poolCap
  ...
  Widget build(BuildContext context) {
    ...
    final capText = isPremiumUser
        ? 'Up to $poolCap'
        : 'Free up to ${math.min(freeMaxExamQ, poolCap)}';
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              ...
              // ✅ new: view button styled like a stat (no left icon, arrow right)
              _ActionStat(
...
class StatusHintCompactCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (totalSelected > pool) {
      ...
      title = 'Tip';
      message =
      'Use the + / − buttons to adjust MCQ and SBA in steps of 5.';
...
class _TypeStepperTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final border =
    selected ? AppColor.indigo.withOpacity(0.25) : Colors.black.withOpacity(0.06);
...
class _ConstraintBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (!okMin) {
      ...
      tint = const Color(0xFFF59E0B);
      label = isPremiumUser ? 'Max $planMax' : 'Free max $freeMax (Premium for more)';
    } else {
...
/// ✅ summary “View” tile inside Wrap, looks like a stat, arrow right
